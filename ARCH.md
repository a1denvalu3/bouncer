...
3. **Backfill Dispatcher (`backfill.sh`)**: A historical chained-scan mode, invoked manually (`docker compose run`), that reviews every **merged** PR of a repository sequentially (oldest to newest) within an optional start/end window (PR numbers or dates). It reuses the exact same workspace preparation, nspawn sandbox, and runner as the other modes, but carries a **findings ledger** forward between reviews (stored encrypted in the `backfill_ledger` table). Each review's prompt receives the ledger as prior-findings context (dedup, invalidation tracking), and the verifier maintains it under the `FINDINGS_BUDGET` cap — when exceeded, the least important entries (lowest severity, `theoretical` before `confirmed`, oldest first) are evicted. When a later PR fixes, invalidates, or disproves an earlier finding, the verifier records it in a resolutions file and the orchestrator marks the original report as resolved (`resolved_by_pr`/`resolved_reason` on `pr_reports`); in remote mode the stale report PR in `REPORT_REPO` is additionally closed with a reference to the resolving PR. Resumable via the shared `pr_reviews` state table.
4. **Encrypted SQLCipher Database**: Replaces file-based state tracking. Stores the OIDs of already reviewed commits (`pr_reviews` table) to prevent redundant work, securely ingests generated vulnerability reports and LLM execution metrics (`pr_reports` table), and persists the backfill findings ledger (`backfill_ledger` table). Automatically migrated and provisioned by `migrate_db.sh` using `DB_PASSPHRASE`. Data is persisted to the local `/out/bouncer.db` volume.
5. **systemd-nspawn Sandbox**: An isolated Linux environment (namespace container) instantiated specifically for a single PR review. It is built using an ephemeral RAM disk overlay (`--volatile=overlay`) to ensure it boots instantly and prevents any filesystem state from leaking between reviews.
6. **opencode-ai**: The AI agent executing inside the sandbox. It is given the code diff and instructed to find vulnerabilities, draft Proof of Concepts (PoCs), and test them against the isolated codebase.
